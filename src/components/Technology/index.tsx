...
const TechnologyProficiency = () => {
  ...
  return (
    <section
      id="technologies"
      className="bg-white py-20 md:py-18 lg:py-18 justify-center"
    >
      ...
      <div className="container my-16">
        <ul className="flex list-none justify-evenly items-center text-2xl border-b-[4px] border-gray-200 dark:border-white-300">
          {Object.keys(technologiesData).map((technologyName, index) => (
            <span
              key={`${index}-container`}
              className={`py-2 ${technologyName === activeTab ? "border-b-[2px] border-primary" : ""} hover:border-b-[2px] hover:border-primary`}
            >
              <li
                key={index}
                className={`cursor-pointer hover:text-primary ${technologyName === activeTab ? "text-primary" : ""}`}
                onClick={() => {
                  setActiveTab(technologyName);
                }}
              >
                {technologyName}
              </li>
